...
export class Vec3 extends Float32Array {
   ...
   * @return {Vec3} 面法線ベクトル
   */
  static faceNormal(v0: Vec3, v1: Vec3, v2: Vec3) {
...
export class Mat4 extends Float32Array {
   ...
   * @return {Mat4} 乗算結果の行列
   */
  static multiply(mat0: Mat4, mat1: Mat4, dest?: Mat4) {
    const out = dest === undefined ? Mat4.create() : dest
    const a = mat0[0],
      b = mat0[1],
      c = mat0[2],
   ...
   * @return {Mat4} 結果の行列
   */
  static rotate(mat: Mat4, angle: number, axis: Vec3, dest?: Mat4) {
    let out = dest === undefined ? Mat4.create() : dest
    const sq = Math.sqrt(
      axis[0] * axis[0] + axis[1] * axis[1] + axis[2] * axis[2]
    )
    if (!sq) throw new Error('sq is null')
    let a = axis[0],
      b = axis[1],
      c = axis[2]
    if (sq != 1) {
   ...
   * @return {Mat4} 結果の行列
   */
  static lookAt(eye: Vec3, center: Vec3, up: Vec3, dest?: Mat4) {
    const out = dest === undefined ? Mat4.create() : dest
    const eyeX = eye[0],
      eyeY = eye[1],
      eyeZ = eye[2],
   ...
   * @return {Mat4} 結果の行列
   */
  static transpose(mat: Mat4, dest?: Mat4) {
    const out = dest === undefined ? Mat4.create() : dest
    out[0] = mat[0]
    out[1] = mat[4]
    out[2] = mat[8]
   ...
   * @return {Mat4} 結果の行列
   */
  static inverse(mat: Mat4, dest?: Mat4) {
    const out = dest === undefined ? Mat4.create() : dest
    const a = mat[0],
      b = mat[1],
      c = mat[2],
   ...
   */
  static toVecIV(mat: Mat4, vec: number[]) {
    const a = mat[0],
      b = mat[1],
      c = mat[2],
      d = mat[3],
 ...
 * @class Qtn
 */
export class Qtn extends Float32Array {
  /**
   ...
   * @return {Qtn} 結果のクォータニオン
   */
  static inverse(qtn: Qtn, dest: Qtn) {
    const out = dest == null ? Qtn.create() : dest
    out[0] = -qtn[0]
    out[1] = -qtn[1]
    out[2] = -qtn[2]
   ...
   * @return {Qtn} 結果のクォータニオン
   */
  static multiply(qtn0: Qtn, qtn1: Qtn, dest: Qtn) {
    const out = dest == null ? Qtn.create() : dest
    const ax = qtn0[0],
      ay = qtn0[1],
      az = qtn0[2],
   ...
   * @return {Qtn} 結果のクォータニオン
   */
  static rotate(angle: number, axis: Vec3, dest: Qtn) {
    const out = dest == null ? Qtn.create() : dest
    let a = axis[0],
      b = axis[1],
      c = axis[2]
   ...
   * @return {Vec3} 結果のベクトル
   */
  static toVecIII(vec: Vec3, qtn: Qtn, dest?: Vec3) {
    const out = dest === undefined ? Vec3.create() : dest
    const qp = Qtn.create()
    const qq = Qtn.create()
    const qr = Qtn.create()
    Qtn.inverse(qtn, qr)
   ...
   * @return {Mat4} 結果の行列
   */
  static toMatIV(qtn: Qtn, dest: Mat4) {
    const out = dest == null ? Mat4.create() : dest
    const x = qtn[0],
      y = qtn[1],
      z = qtn[2],
   ...
   * @return {Qtn} 結果のクォータニオン
   */
  static slerp(qtn0: Qtn, qtn1: Qtn, time: number, dest: Qtn) {
    const out = dest == null ? Qtn.create() : dest
    const ht =
      qtn0[0] * qtn1[0] +
      qtn0[1] * qtn1[1] +
