import { Mat4, Qtn, Vec2, Vec3 } from './math.js'
...
export class WebGLOrbitCamera {
  ...
  private defaultUpDirection: Vec3
  private movePosition: Vec3
   ...
   * マウスボタンが押された際のイベント
   */
  mouseInteractionStart(event: MouseEvent) {
    this.isDown = true
   ...
   * マウスが移動した際のイベント
   */
  mouseInteractionMove(event: MouseEvent) {
    if (this.isDown !== true) {
      return
    }
    const bound = this.target.getBoundingClientRect()
